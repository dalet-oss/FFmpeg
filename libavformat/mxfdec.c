...
typedef struct MXFContext {
    ...
    int         growing_pumping;            ///< re-entrancy guard, see mxf_growing_wait_step()
...
#define MXF_GROWING_WAIT_PUMP_PACKETS     256 /* see mxf_growing_wait_step()'s writer pump */
...
static int mxf_growing_wait_step(AVFormatContext *s)
{
    ...
    /* A writer's own index only advances as a side effect of
     * mxf_read_packet() walking forward through KLVs
     * (mxf_growing_index_ref_klv()) - nothing else in this wait loop does
     * that. A writer blocked here on a seek whose target is ahead of
     * wherever normal delivery has already reached would otherwise wait
     * forever: the file keeps growing on disk, but nothing ever reads it
     * forward to index it. Pump forward by calling mxf_read_packet() and
     * discarding the result - safe because mxf_read_seek()'s VBR path does
     * its own explicit absolute avio_seek() and recomputes every track's
     * sample_count once this wait returns, so nothing pumping leaves
     * behind in either the AVIO position or sample_count is depended upon.
     *
     * growing_pumping guards against this call re-entering itself:
     * mxf_read_packet() has its own call site of mxf_growing_wait_step()
     * (waiting for the next KLV to arrive), and pumping from inside a pump
     * would recurse without ever making bounded progress - the reentrant
     * call just falls through to the plain sleep/probe wait below. */
    if (mxf->growing_role == MXF_GROWING_ROLE_WRITER && !mxf->growing_pumping) {
        int i, pumped = 0;

        mxf->growing_pumping = 1;
