...
typedef struct MXFContext {
    ...
    /* TEMP DIAGNOSTIC (RDC-15459): set by a growing seek, consumed by the
     * very next klv_read_packet() so the seek's actual landing spot is
     * observed via the read that would happen anyway - no extra I/O added. */
    int         growing_diag_pending;
...
static int klv_read_packet(MXFContext *mxf, KLVPacket *klv, AVIOContext *pb)
{
    int64_t length, pos;
    int llen;
    /* TEMP DIAGNOSTIC (RDC-15459): consume any pending growing-seek report
     * on this, the very next read after that seek - purely observational,
     * no extra I/O beyond avio_tell() (in-memory, no syscall). */
    int diag_pending = mxf->growing_diag_pending;
    int64_t diag_target = mxf->growing_diag_target;
    int64_t diag_pos_before = diag_pending ? avio_tell(pb) : 0;
    mxf->growing_diag_pending = 0;

    if (!mxf_read_sync_klv(pb)) {
        if (diag_pending)
            av_log(mxf->fc, AV_LOG_WARNING,
                   "growing MXF DIAG: seek target=%"PRId64" pos_before=%"PRId64
                   " - sync_klv found no KLV key (hit EOF first)\n",
                   diag_target, diag_pos_before);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (diag_pending)
        av_log(mxf->fc, AV_LOG_WARNING,
               ...
               diag_target, diag_pos_before, klv->offset, klv->offset - diag_target);
...
#define MXF_GROWING_ENOENT_FOOTER_RETRIES  5 /* see mxf_growing_reader_wait_step() ENOENT handling */
#define MXF_GROWING_SIDECAR_CATCHUP_RETRIES 5 /* see mxf_read_seek()'s VBR entries check */
#define MXF_GROWING_WAIT_PUMP_PACKETS     256 /* see mxf_growing_wait_step_for_seek() */
...
static void mxf_growing_resync_tracks(AVFormatContext *s, int64_t ref_offset);
static void mxf_growing_republish_sidecar(AVFormatContext *s);
...
static int mxf_growing_measure_stride(AVFormatContext *s, int64_t scan_from)
{
    ...
    while (n < 3 && scanned++ < MXF_GROWING_STRIDE_SCAN_KLVS) {
        ...
        if (klv_read_packet(mxf, &klv, s->pb) < 0)
            break;
        /* Same OP1a offset fix as mxf_growing_index_ref_klv()/
         * mxf_read_packet(): a content package's system item, when present,
         * precedes the reference track's own element within it, so this
         * independent scan (it does not go through mxf_read_packet(), which
         * is where growing_cp_start_offset normally gets tracked) must track
         * the same boundary itself, or growing_essence_offset ends up
         * mid-package - wrong for the seek-to-0 fast path and for a restart
         * or republish that reseeds from it. */
        if (cp_start < 0 &&
        ...
        if (mxf_is_essence_element_key(klv.key)) {
            ...
            if (mxf_get_stream_index(s, &klv, body_sid) == mxf->growing_ref_stream) {
                starts[n] = (cp_start >= 0 && cp_start <= klv.offset) ? cp_start : klv.offset;
                sizes[n]  = klv.next_klv - klv.offset;
                n++;
                cp_start = -1;  /* consumed for this content package */
            }
        }
...
static int64_t mxf_growing_available_edit_units(AVFormatContext *s)
{
    ...
    if (mxf->growing_role == MXF_GROWING_ROLE_READER) {
         ...
         *
         * The writer updates the duration field and appends dense entries as
         * two separate, unordered writes, so the duration field can be ahead
         * of what mxf_read_seek()'s VBR path can actually index into
         * (gi->offsets[]). Once a dense index exists, it - not the coarser
         * duration field - is the only thing a seek can use, so it must be
         * the availability signal too: FFMAX-ing in the duration field here
         * lets a wait release before the entries a seek needs have landed. */
        ...
        else if (mxf->growing_sidecar_duration > 0)
            idx = mxf->growing_sidecar_duration;
        return idx;
    }
...
static int mxf_growing_wait_step(AVFormatContext *s)
{
    ...
    if (now - mxf->growing_last_probe_us >= MXF_GROWING_FOOTER_PROBE_US) {
        ...
        /* Nothing in the normal per-edit-unit write path (write_index_entry(),
         * patch_sidecar_duration()) re-reads or re-stats growing_index_file -
         * they all write through the already-open handle, which POSIX happily
         * keeps accepting writes on even after the path itself is unlinked. A
         * solo writer (nobody ever raced for the lock, the common case) would
         * otherwise have no way to learn its sidecar was removed by something
         * else and would keep writing into that orphaned, invisible inode
         * forever. fstat()'s link count is the direct way to ask "is my own
         * open handle still reachable by path" - cheaper and race-free
         * compared to a separate access()/stat() on the path. */
#if HAVE_UNISTD_H
        if (mxf->growing_index_out && !mxf->growing_index_disabled) {
            struct stat st;

            if (fstat(fileno(mxf->growing_index_out), &st) == 0 && st.st_nlink == 0)
                mxf_growing_republish_sidecar(s);
        }
#endif
    }
...
/**
 * Like mxf_growing_wait_step(), but additionally pumps the writer's own
 * forward-parsing while waiting, by calling mxf_read_packet() and
 * discarding the result. A writer's own dense index only advances as a
 * side effect of mxf_read_packet() walking forward through KLVs
 * (mxf_growing_index_ref_klv()) - nothing in the plain wait step does that
 * - so a writer blocked on a seek whose target is ahead of wherever
 * normal delivery has already reached would otherwise wait forever: the
 * file keeps growing on disk, but nothing ever reads it forward to index
 * it.
 *
 * ONLY safe to call from mxf_growing_wait_for_edit_unit()'s seek-wait,
 * where no caller is currently waiting on mxf_read_packet() to return a
 * specific packet: pumping there discards whatever mxf_read_packet()
 * returns, and mxf_read_seek()'s VBR/CBR paths both do their own explicit
 * absolute avio_seek() and recompute every track's sample_count once this
 * wait returns, so nothing pumping leaves behind in either the AVIO
 * position or sample_count is depended upon. mxf_read_packet()'s own
 * "wait for the next KLV to arrive" call site (and
 * mxf_growing_wait_for_data()'s clip-wrapped equivalent) must keep using
 * the plain mxf_growing_wait_step() above unchanged - pumping there would
 * silently steal the very packet that call is waiting to deliver to its
 * caller, corrupting playback (observed as "No PTS AND no DTS" once this
 * was tried unconditionally). mxf_read_packet() itself may still recurse
 * into the plain wait step from inside this pump's own calls - that is
 * fine, it never re-enters this function.
 */
static int mxf_growing_wait_step_for_seek(AVFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    ...
    if (mxf->growing_role == MXF_GROWING_ROLE_WRITER) {
        ...
        for (i = 0; i < MXF_GROWING_WAIT_PUMP_PACKETS; i++) {
            AVPacket *pkt = av_packet_alloc();
            int ret;

            if (!pkt)
                break;
            ret = mxf_read_packet(s, pkt);
            av_packet_free(&pkt);
            if (ret < 0)
                break;
            pumped = 1;
        ...
        if (pumped) {
            mxf_growing_note_progress(mxf);
            return MXF_WAIT_RETRY;
        }
    }

    return mxf_growing_wait_step(s);
}
...
/**
 * The writer's sidecar was found unlinked out from under it (see the
 * fstat() check in mxf_growing_wait_step()) by something other than its own
 * completion path (mxf_growing_transition_to_closed() deletes it only after
 * this process has already reverted to non-growing). Unlike a reader
 * recovering from the same kind of event
 * (mxf_growing_reassign_after_sidecar_loss()), this writer's own AVIO
 * position and in-memory growing_vbr_index were never touched - only the
 * on-disk coordination file was - so recovery is republishing that
 * already-correct state to a freshly created file at the same path, never a
 * restart. A solo writer (the common case: nobody else ever raced for the
 * lock) had no other way to learn its sidecar had vanished at all - nothing
 * in its normal per-edit-unit write path re-reads or re-stats the path
 * itself, unlike a reader's reload_sidecar().
 */
static void mxf_growing_republish_sidecar(AVFormatContext *s)
{
    MXFContext *mxf = s->priv_data;
    MXFGrowingIndex *gi = mxf->growing_vbr_index;
    int64_t i, nb_entries = gi ? gi->nb_entries : 0;

    mxf_growing_release_sidecar_write(mxf);
    if (mxf_growing_open_sidecar_write(s, 1) < 0 || !mxf->growing_index_out) {
        av_log(s, AV_LOG_WARNING, "growing MXF: sidecar %s was removed and "
               "could not be recreated\n", mxf->growing_index_file);
        return;
    }
    for (i = 0; i < nb_entries && mxf->growing_index_out; i++)
        mxf_growing_write_index_entry(s, gi->offsets[i], gi->sizes[i],
                                      gi->temporal_offsets[i], gi->flags[i]);
    mxf_growing_patch_sidecar_duration(s, mxf->growing_cur_duration);
    av_log(s, AV_LOG_INFO, "growing MXF: sidecar %s was removed; recreated "
           "with %"PRId64" existing entries\n", mxf->growing_index_file,
           nb_entries);
}
...
static int mxf_growing_attempt_takeover(AVFormatContext *s)
{
    ...
    /* This reader's own sequential delivery is gated only on raw byte
     * presence (mxf_growing_incomplete()), never on the sidecar frontier, so
     * by the time a stall makes it attempt takeover it may already have
     * delivered packets past growing_index_resume_ofs. Reseeking AVIO back
     * there without also resyncing every track's sample_count would leave
     * mxf_read_packet()'s own bookkeeping referring to wherever it had
     * already reached, mismatched against the rewound cursor. */
    mxf_growing_resync_tracks(s, mxf->growing_vbr_index->nb_entries);
    mxf->growing_last_progress_us = av_gettime_relative();
...
static int mxf_growing_reassign_after_sidecar_loss(AVFormatContext *s)
{
    ...
    if (mxf->growing_role == MXF_GROWING_ROLE_WRITER) {
        ...
        if (avio_seek(s->pb, mxf->growing_essence_offset, SEEK_SET) < 0)
            return AVERROR(EIO);
        mxf->current_klv_data = (KLVPacket){{0}};
        /* Same reasoning as mxf_growing_attempt_takeover(): this reader may
         * already have sequentially delivered packets past edit unit 0, so
         * every track's sample_count must be rewound to match the AVIO
         * cursor, not just left wherever sequential delivery had reached. */
        mxf_growing_resync_tracks(s, 0);
        mxf->growing_last_progress_us = av_gettime_relative();
...
static int mxf_growing_reader_wait_step(AVFormatContext *s)
{
    ...
    if (reload == AVERROR(ENOENT)) {
        ...
        /* The sidecar being gone does not by itself prove the footer is
         * visible to THIS process yet. The writer deletes it as soon as it
         * sees the footer partition pack key in its own sequential KLV walk
         * - needing only bytes up to that key - but this reader's only way
         * to confirm the footer (mxf_growing_probe_footer(), via the
         * trailing RandomIndexPack; see its own comment on why the header-
         * patch signal never applies to a file that started non-seekable)
         * needs the file's very last bytes, which the real external writer
         * can still be mid-flush of at that exact instant. Concluding
         * "genuinely removed by something else" on a single failed probe is
         * destructive - mxf_growing_reassign_after_sidecar_loss() rewinds
         * everything to edit unit 0 - so give the race a short bounded
         * chance to resolve first. */
        for (int i = 0; fp == 0 && i < MXF_GROWING_ENOENT_FOOTER_RETRIES; i++) {
            av_usleep(MXF_GROWING_SLEEP_SLICE_US);
            if (ff_check_interrupt(&s->interrupt_callback))
                return MXF_WAIT_INTERRUPT;
            fp = mxf_growing_probe_footer(s);
        }
    ...
    /* Deliberately NO reader-side raw footer probe here (unlike
     * mxf_growing_wait_step()'s writer path). dev-note-growing-mxf.md's
     * completion protocol is: the writer finishes indexing to the true end,
     * THEN deletes the sidecar - deletion is the sole completion signal, and
     * the reader only probes the footer itself after reload_sidecar() above
     * returns ENOENT (i.e. after that deletion has already happened). A
     * probe on this function's own cadence would let the reader see the
     * footer's raw bytes - which can land on disk well before the writer
     * finishes catching the sidecar up - and declare avail final while the
     * sidecar is still mid catch-up, e.g. "seek target edit unit 250 is past
     * the final duration 42" when 42 is only what the sidecar had indexed at
     * that instant. If an incumbent writer dies right at the footer without
     * deleting the sidecar, the stall/takeover path above is what recovers:
     * this reader takes over as writer, finishes indexing for real, and only
     * then deletes it. */
...
static int mxf_growing_transition_to_closed(AVFormatContext *s)
{
    ...
    if (mxf->footer_partition) {
        /* Walk every partition from the footer back to (but excluding) the
         * header, collecting each one's own IndexTableSegment(s). A growing
         * OP1a mux flushes essence and index interleaved, partition by
         * partition, as it writes - the footer's own segment only covers
         * the span since the last flush, so parsing just the footer (as
         * this function used to) publishes a duration truncated to that
         * last flush instead of the whole file. The header partition is
         * still never revisited: its structural metadata was already
         * parsed at open time and re-parsing it here would duplicate the
         * streams created from it (see the function comment above).
         *
         * mxf_read_partition_pack() is deliberately allowed to run here
         * (via the generic dispatch below, since it is registered in
         * mxf_metadata_read_table like any other metadata set) rather than
         * hand-parsed: with mxf->parsing_backward set, it inserts each
         * newly-discovered partition at mxf->last_forward_partition - the
         * slot right after the header, which is all a growing-mode open
         * ever forward-parses - so repeatedly discovering partitions in
         * reverse chronological order still leaves mxf->partitions[]
         * sorted by file offset, exactly as mxf_compute_essence_containers()
         * requires.
         *
         * This function has more than one call site and is not itself
         * gated on having run before, so a second call must not re-append
         * every partition on top of what the first call already inserted -
         * that left two duplicate entries (same pack_ofs) in
         * mxf->partitions[] and broke the ascending-offset invariant
         * mxf_compute_essence_containers() relies on (observed as a
         * bogus "bad ThisPartition" error, harmless only because it
         * clamps that one partition's essence_length to 0 instead of
         * failing outright). Discard anything past the header before
         * walking again, exactly as the index-table rebuild below already
         * does unconditionally on every call. */
        uint64_t partition_offset = mxf->footer_partition;
        int iterations;
        int saved_parsing_backward = mxf->parsing_backward;

        mxf->partitions_count      = FFMIN(mxf->partitions_count, 1);
        mxf->last_forward_partition = mxf->partitions_count;
        mxf->parsing_backward = 1;

        for (iterations = 0; iterations < mxf->partitions_count + 4096; iterations++) {
            uint64_t next_partition;

            ret = avio_seek(s->pb, mxf->run_in + partition_offset, SEEK_SET);
            if (ret < 0)
                break;
            mxf->current_partition = NULL;
            ...
            while (!avio_feof(s->pb)) {
                ...
                if (ret < 0)
                    break;
                if (IS_KLV_KEY(klv.key, ff_mxf_random_index_pack_key))
                    break;
                if (mxf_is_essence_element_key(klv.key)) {
                    /* same first-essence-KLV bookkeeping mxf_read_header()
                     * does, needed by mxf_compute_essence_containers() */
                    if (mxf->current_partition &&
                        !mxf->current_partition->first_essence_klv.offset)
            ...
            /* no PartitionPack found at partition_offset - malformed */
            if (!mxf->current_partition)
                break;

            /* previous_partition == 0 means the header - stop without
             * revisiting it; it must also strictly decrease to guard
             * against a corrupt PreviousPartition cycle, exactly like
             * mxf_seek_to_previous_partition()'s own equivalent check */
            next_partition = mxf->current_partition->previous_partition;
            if (next_partition == 0 || next_partition >= partition_offset)
                break;
            partition_offset = next_partition;
        }

        mxf->parsing_backward = saved_parsing_backward;
        ...
        /* mxf->partitions[] now covers every partition instead of just the
         * header, so the essence-container offsets latched at open time
         * (from the header alone) are stale - without this,
         * mxf_edit_unit_absolute_offset() cannot resolve any edit unit
         * whose essence landed in a later partition. */
 ...
 *
 * INVARIANT: on every return path except the growing_file_closed one below,
 * this mutates no demuxer state other than the growing-mode duration and
 * index caches. The AVIO position, mxf->current_klv_data, every
 * track->sample_count and every cur_dts are untouched on every return path
 * (including that one - mxf_growing_transition_to_closed() saves/restores the
 * AVIO position itself), so a subsequent mxf_read_packet() resumes exactly
 * where it was. That is what makes returning AVERROR_EOF from here safe. Do
 * NOT be tempted to clamp sample_time in here.
 *
 * The growing_file_closed path DOES rebuild mxf->index_tables and republish
 * every stream's duration (via mxf_growing_transition_to_closed()) - safe
 * only because mxf_read_seek(), our sole caller, does not take a pointer into
 * mxf->index_tables until after this function returns (see the comment at
 * its call site). Do not call this from anywhere else without re-checking
 * that.
 *
                                          ...
                                          int64_t sample_time)
{
    ...
    for (;;) {
        ...
        if (mxf->growing_file_closed) {
            ...
            /* Do not trust avail here. For a reader it is only ever as
             * fresh as the sidecar's last successful reload, which can
             * predate the writer's very last duration patch - the sidecar
             * cannot be re-read once mxf_growing_reload_sidecar() has
             * observed it gone (ENOENT). The underlying file is provably no
             * longer growing at this point (that is what growing_file_closed
             * means), so re-derive duration from it directly instead of
             * trusting a stale snapshot: mxf_growing_transition_to_closed()
             * rebuilds mxf->index_tables from the real footer index and
             * republishes every track's true original_duration. */
            ret = mxf_growing_transition_to_closed(s);
            if (ret < 0)
                return ret;
            final_dur = ref->original_duration;
            if (ref_target < final_dur)
                return 0;
            av_log(s, AV_LOG_ERROR, "growing MXF: seek target edit unit "
                   "%"PRId64" is past the final duration %"PRId64"\n",
                   ref_target, final_dur);
            return AVERROR_EOF;
        }
        if (avail > prev_avail) {
            prev_avail = avail;
        ...
        switch (mxf->growing_role == MXF_GROWING_ROLE_READER
                ? mxf_growing_reader_wait_step(s)
                : mxf_growing_wait_step_for_seek(s)) {
        case MXF_WAIT_RETRY:
...
/**
 * Resync every stream's sample_count to a reference-track edit unit that
 * mxf->pb's position has just been (re)set to the start of - needed after a
 * mid-run role reassignment (mxf_growing_attempt_takeover() /
 * mxf_growing_reassign_after_sidecar_loss()) rewinds the shared AVIO cursor
 * without otherwise touching this bookkeeping.
 *
 * Deliberately NOT mxf_get_next_track_edit_unit() (what mxf_read_seek()'s
 * growing-VBR branch uses for the same kind of resync): that searches the
 * container's own *published* index tables, which for a still-growing VBR
 * reference track are exactly what "broken or empty index" at open means -
 * there is nothing there to search yet. Plain edit-rate rescaling against
 * the reference track is what the rest of the growing subsystem already
 * uses for this same track-to-track mapping (see the per-track loop in
 * mxf_growing_refresh_duration()), so it stays correct through the same
 * growing-mode window this whole subsystem exists for.
 */
static void mxf_growing_resync_tracks(AVFormatContext *s, int64_t ref_edit_unit)
{
    MXFContext *mxf = s->priv_data;
    MXFTrack *ref = mxf_growing_ref_track(s);
    ...
    for (int i = 0; i < s->nb_streams; i++) {
        ...
        if (!cur_track || !cur_track->edit_rate.num)
            continue;
        track_edit_unit = av_rescale_q(ref_edit_unit, av_inv_q(ref->edit_rate),
...
/**
 * True when a reader's own sequential read has reached a reference-track
 * edit unit whose bytes are already on disk but which the sidecar - built
 * by whichever process currently holds the writer role - has not indexed
 * yet, so mxf_growing_set_reordered_pts() cannot resolve it and would leave
 * pts == AV_NOPTS_VALUE.
 *
 * mxf_growing_incomplete() only ever waits for raw bytes: it has no idea the
 * sidecar exists. A reader's ordinary sequential delivery is gated solely on
 * those raw bytes too (mxf_growing_wait_for_edit_unit() only gates seeks),
 * so nothing stops it running ahead of a writer process that is still
 * catching its own forward parse - and therefore its sidecar writes - up to
 * where the reader has already reached by just reading whatever is on disk.
 * Confirmed against the multi-process harness (multi.py Check 1): with two
 * concurrent ffprobe processes sharing a growing MPEG-2 file, the reader's
 * packets matched the writer's byte-for-byte (same pos/size/flags) but a
 * chunk of them came back pts=N/A where the writer had a real reordered
 * value, for exactly this reason.
 *
 * Writer role needs no equivalent check: mxf_growing_index_ref_klv() indexes
 * this edit unit earlier in the same mxf_read_packet() walk, always before
 * mxf_set_pts() is reached for it.
 */
static int mxf_growing_reorder_pending(AVFormatContext *s, AVStream *st,
                                       MXFTrack *track, int64_t edit_unit)
{
    MXFContext *mxf = s->priv_data;
    enum AVCodecID cid;
    ...
    if (mxf->growing_role != MXF_GROWING_ROLE_READER)
        return 0;
    if (!track || st->index != mxf->growing_ref_stream || track->intra_only)
        return 0;
    cid = st->codecpar->codec_id;
    if (cid != AV_CODEC_ID_MPEG2VIDEO && cid != AV_CODEC_ID_H264)
        return 0;

    mxf_growing_reload_sidecar(s);
    return !mxf->growing_vbr_index || edit_unit >= mxf->growing_vbr_index->nb_entries;
}
...
static int mxf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (1) {
        ...
        if (mxf_match_uid(klv.key, mxf_essence_element_key, 12) ||
             ...
             * written; klv.next_klv is the real packet end here. Also wait
             * if the bytes are there but the sidecar has not indexed this
             * edit unit yet - see mxf_growing_reorder_pending(). */
            if (mxf->growing && track && track->wrapping == FrameWrapped &&
                (mxf_growing_incomplete(s, klv.next_klv) ||
                 mxf_growing_reorder_pending(s, st, track, track->sample_count))) {
...
static int mxf_read_seek(AVFormatContext *s, int stream_index, int64_t sample_time, int flags)
{
    ...
    /* growing CBR: the target's byte offset is pure geometry once stride and
     * essence_offset are known (measured by the writer, or loaded from the
     * sidecar header by a reader - either way, role-independent). No dense
     * index is needed or waited on: mxf_growing_index_ref_klv() only
     * appends entries as a side effect of normal forward reads, and for CBR
     * content those can lag far behind what raw file size alone already
     * proves is on disk - which is exactly what
     * mxf_growing_wait_for_edit_unit() above already confirmed via
     * mxf_growing_compute_duration()'s byte-based measure. Requiring the
     * dense index to also have caught up (the growing-VBR branch below)
     * left a CBR seek waiting on an index nothing was advancing. */
    if (mxf->growing && mxf->growing_stride > 0 && !mxf->growing_clip_wrapped) {
        MXFTrack *ref = mxf_growing_ref_track(s);

        if (ref && ref->edit_rate.num && source_track->edit_rate.num) {
            int64_t ref_eu = av_rescale_q(FFMAX(sample_time, 0),
                                          av_inv_q(source_track->edit_rate),
                                          av_inv_q(ref->edit_rate));
            int64_t target_offset = mxf->growing_essence_offset +
                                     ref_eu * mxf->growing_stride;

            seekpos = avio_seek(s->pb, target_offset, SEEK_SET);
            if (seekpos < 0)
                return seekpos;

            /* TEMP DIAGNOSTIC (RDC-15459): let the very next klv_read_packet()
             * report where this seek actually landed, using the read that
             * would happen anyway - adds no extra I/O of its own. */
            mxf->growing_diag_pending = 1;
    ...
    if (mxf->growing && mxf->growing_vbr_index) {
        ...
        if (gi->nb_entries > 0 && mxf->nb_index_tables <= 0 && ref &&
            ...
            if (ref_eu >= gi->nb_entries) {
                /* mxf_growing_reload_sidecar() only ever appends to
                 * gi->nb_entries, so this is not a shrink - the wait was
                 * satisfied against mxf_growing_available_edit_units(),
                 * which for a reader can be ahead of the dense entries
                 * (see its comment); give the entries a short bounded
                 * chance to catch up before giving up. */
                int i;

                for (i = 0; ref_eu >= gi->nb_entries &&
                            i < MXF_GROWING_SIDECAR_CATCHUP_RETRIES; i++) {
                    av_usleep(MXF_GROWING_SLEEP_SLICE_US);
                    if (ff_check_interrupt(&s->interrupt_callback))
                        return AVERROR_EXIT;
                    mxf_growing_reload_sidecar(s);
                ...
                if (ref_eu >= gi->nb_entries) {
                    av_log(s, AV_LOG_ERROR, "growing MXF: sidecar entries did "
                           "not catch up during the seek (%"PRId64" entries, "
