...
static void *open_callback(const char* utf8Path, void *data)
{
    ...
    av_dict_copy(&tmp, ctx->avio_opts, 0);

    if (avio_open2(&related_redc_file_ctx->pb, utf8Path, AVIO_FLAG_READ, NULL, &tmp) < 0) {
...
static int redc_read_header(AVFormatContext *avctx) {
    ...
    if ((ret = ffio_copy_url_options(avctx->pb, &ctx->avio_opts)) < 0)
        return ret;
    ...
    if (info->start_edge_timecode) {
        av_dict_set(&avctx->metadata, "timecode", info->start_edge_timecode, 0);
    }

    // set each of the info metadata fields as well
    for (int i = 0; i < info->num_metadata_entries; i++) {
        av_dict_set(&avctx->metadata, info->metadata[i].key, info->metadata[i].value, 0);
    }
...
static int redc_read_video_packet(AVFormatContext *s, AVPacket *pkt) {
    ...
    av_log(s, AV_LOG_TRACE, "video_ctx->current_frame_offset: %lld\n", video_ctx->current_frame_offset);
    av_log(s, AV_LOG_TRACE, "video_ctx->current_time_offset: %lld\n", video_ctx->current_time_offset);
...
static int redc_read_audio_packet(AVFormatContext *s, AVPacket *pkt) {
    ...
    av_log(s, AV_LOG_TRACE, "audio_ctx->current_sample_offset: %lld\n", audio_ctx->current_sample_offset);
    av_log(s, AV_LOG_TRACE, "audio_ctx->current_time_offset: %lld\n", audio_ctx->current_time_offset);
...
static int redc_probe(const AVProbeData *p) {
    ...
    if (AV_RL32(p->buf + 4) == MKTAG('R','E','D','2') ||
        (AV_RL32(p->buf + 4) == MKTAG('f','t','y','p') && AV_RL32(p->buf + 8) == MKTAG('n','i','k','o'))) {
...
static int redc_seek(AVFormatContext *s, int stream_index, int64_t sample_time, int flags) {
    ...
    AVStream *video = NULL;
    AVStream *audio = NULL;
    LibRedcVideoContext *video_ctx = NULL;
    LibRedcAudioContext *audio_ctx = NULL;

    av_log(ctx, AV_LOG_DEBUG, "redc_seek: %ld\n", sample_time);

    video = s->streams[0];
    video_ctx = video->priv_data;

    video_ctx->current_frame_offset = sample_time;
    video_ctx->current_time_offset = av_rescale_q(
        (int64_t)video_ctx->current_frame_offset,
        video->time_base,
        (AVRational){1, AV_TIME_BASE}
    ...
    if (s->nb_streams > 1) {
        audio = s->streams[1];
        audio_ctx = audio->priv_data;

        audio_ctx->current_sample_offset = av_rescale_rnd(sample_time,
            audio->time_base.num,
            audio->time_base.den,
