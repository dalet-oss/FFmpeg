                                ...
                                int final)
{
    ...
    int i, ret;
    int has_representation = 0;

    /* A text representation that never landed a single real segment (e.g. a clip with
     * genuinely no cues in it) would otherwise be declared with a permanently empty
     * <SegmentTimeline>, which crashes any reader that opens the manifest. Skip the
     * whole AdaptationSet rather than publish one with no usable representations - only
     * once final, since a live/dynamic manifest may still go on to produce segments for
     * a representation that simply hasn't written one yet. */
    for (i = 0; i < s->nb_streams; i++) {
        OutputStream *os = &c->streams[i];
        if (os->as_idx - 1 != as_index)
            continue;
        if (final && as->media_type == AVMEDIA_TYPE_SUBTITLE && !os->nb_segments)
            continue;
        has_representation = 1;
        break;
    }
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (final && as->media_type == AVMEDIA_TYPE_SUBTITLE && !os->nb_segments)
            continue; // no cues ever landed in a real segment - omit rather than declare a permanently-broken representation
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (is_text_in_mp4(st, os)) {
            ...
            if (stream >= 0) {
                ...
                // final flush: cover whatever is left of the cues. Before this
                // representation's init segment has ever been flushed, max_pts still
                // holds the raw extent of received cue content rather than a
                // "flushed up to here" marker (that snapping only happens above, once
                // a reference-stream-driven flush has actually run) - using it as the
                // start of "what's left" made seg_end == seg_start trivially, so a
                // reference stream with no further keyframe boundary inside the clip
                // silently dropped every real cue this representation ever received.
                seg_start = os->init_range_length ?
                            (os->max_pts != AV_NOPTS_VALUE ? os->max_pts : 0) :
                            os->start_pts;
